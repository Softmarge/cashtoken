...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAuthSuccess: (demoUser?: UserProfile) => void;
}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex border-b border-gray-100">
          <button
            ...
            className={`flex-1 py-3 text-sm font-semibold transition-colors ${
              mode === 'login' ? 'text-[#7B0F14] border-b-2 border-[#7B0F14]' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            Sign In
          </button>
          <button
            ...
            className={`flex-1 py-3 text-sm font-semibold transition-colors ${
              mode === 'signup' ? 'text-[#7B0F14] border-b-2 border-[#7B0F14]' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            Create Account
          </button>
        ...
        {/* Form */}
        <form onSubmit={mode === 'login' ? handleLogin : handleSignUp} className="px-8 py-6 space-y-4">
          {error && (
          ...
          {mode === 'signup' && (
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g. Michael Johnson"
          ...
          <button
            ...
            className={`w-full py-3.5 rounded-xl font-semibold text-white transition-all shadow-lg ${
              loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#7B0F14] hover:bg-[#5A0B10] active:scale-[0.98]'
            }`}
          >
            {loading ? (
          ...
          <div className="bg-[#F4E6E6]/60 rounded-xl px-4 py-3 text-xs text-gray-500 text-center">
            Demo: <span className="font-semibold text-[#7B0F14]">Alex Johnson</span> &nbsp;|&nbsp; demo@cashtoken.uk
          </div>

          <p className="text-center text-xs text-gray-400 mt-2">
            {mode === 'login' ? (
              <>Don't have an account?{' '}
                <button type="button" onClick={() => switchMode('signup')} className="text-[#7B0F14] font-semibold hover:underline">Sign up free</button>
              </>
            ) : (
              <>Already have an account?{' '}
                <button type="button" onClick={() => switchMode('login')} className="text-[#7B0F14] font-semibold hover:underline">Sign in</button>
              </>
            )}
          </p>
        </form>
